...
def get_mean_distance(name):
    dists = get_raw_distances(name, quiet=1)
    s, n = 0, len(dists)
    for obj1, obj2, dist in dists:
        s += dist
    print('Mean distance: {}, total contacts: {}'.format(s / n, n))
    return s / n

cmd.extend('get_mean_distance', get_mean_distance)
